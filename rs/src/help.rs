...
//! The `--help` / `-h` text. It mirrors the TypeScript `help()`, with a
//! Rust-specific Plugins section: loading a plugin module by reference is
//! not available here (see [`crate::registry`]), so `-p` resolves against
//! the compiled-in registry instead.
...
Plugins
  Plugins are compiled into the binary (Rust cannot load modules at runtime).
  ...
  tabnas_jsonic_cli crate before calling run:
    tabnas_jsonic_cli::register_plugin("csv", tabnas_csv::plugin());
