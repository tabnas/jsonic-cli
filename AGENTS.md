...
`@tabnas/jsonic-cli` to npm over GitHub OIDC trusted publishing (no token,
...
2. Verify against the **published** dependencies rather than your checkout.
   ...
   **Build first.** This package's `npm test` neither has a `pretest` build
   ...
   `dist/` — stale output, or nothing at all on a fresh checkout.
...
6. Confirm — and make the check **fail**, not merely print:
   ...
   ```bash
   V=x.y.z
   npm view @tabnas/jsonic-cli@$V version
   GH=$(npm view @tabnas/jsonic-cli@$V gitHead)
...
- `publish-go V=x.y.z` breaks the version invariant: it `sed`s and stages
  **only** `go/cmd/jsonic/main.go`, leaving `ts/package.json` and `VERSION`
  in `ts/src/jsonic-cli.ts` on the previous version — the exact state the
