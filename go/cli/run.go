...
// Package cli implements the jsonic command line tool as an importable
// library: the standard binary (cmd/jsonic) is a thin wrapper around
// [Run], and custom binaries can compile in additional plugins for
// -p/--plugin via [RegisterPlugin] before calling [Run].
//
...
// load a plugin module by name at runtime. Instead, -p/--plugin resolves
// plugin reference names against a registry of compiled-in plugins. The
// plugins already in this module's dependency graph are pre-registered
// (see registry.go); anything else must be compiled into a custom binary:
//
//	package main
//
//	import (
...
//	}
//
// Everything else — argument parsing, option/meta wiring, source merging,
// and JSON serialization — mirrors the TypeScript CLI exactly.
package cli
...
// TypeScript CLI. extra adds plugins for this run on top of the registered
// ones (same-name entries win over the registry; nil for the standard
// binary). It returns the process exit code.
func Run(argv []string, stdin string, out io.Writer, extra map[string]tabnas.Plugin) int {
	c := &logger{w: out}
	registry := Plugins()
	for name, p := range extra {
		registry[name] = p
	}
	return runLog(argv, stdin, c, registry)
}
...
// the captured lines directly. plugins is the full registry of compiled-in
// plugins available to -p/--plugin for this run.
func runLog(argv []string, stdin string, c *logger, plugins map[string]tabnas.Plugin) int {
	...
	// Resolve named plugins from the registry. An unknown name is an
	// error (the TS CLI throws on a failed require). Plugin options
