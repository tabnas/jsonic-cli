...
for sibling in parser jsonic json debug support; do
...
# two grammars, the debug plugin, the support crate). Those entries
# legitimately move whenever the sibling checkout does, and exempting
# exactly them is what makes a full comparison usable here when blanket
# `--locked` is not.
...
# NOT `--locked`, deliberately, and this is the one place this gate
